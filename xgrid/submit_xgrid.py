...
def API_start_map_construction(url: str, token: str, map_name: str) -> str:
    ...
    data = {
        ...
        "name": map_name,
        "preservePoses": True,
        "featureCount": 4096, #default: 1024
...
def parse_trajectory_csv(csv_filepath: str) -> list[dict]:
    data_list = []
            ...
            for i in range(len(fields)):
                try:
                    ...
                    if field_name == "image_name":
                        processed_row[field_name] = row[i].split("/")[-1]
                    else:
                        processed_row[field_name] = field_type(row[i])
                except ValueError:
                    print(f"Warning: ValueError when processing line {csv_reader.line_num}")
                    return

            processed_row = add_rot_matrix_from_quat(processed_row)
...
def main(input_trajectory_file: str, images_directory: str,  url: str, token: str, map_name: str) -> None:
    ...
    # prepare all data
    data_list = parse_trajectory_csv(input_trajectory_file)
    data_list = append_data_with_image_path(data_list, images_directory)
...
if __name__ == '__main__':
    ...
    input_trajectory_file = r"<path-to-trajectory-file>"
    images_directory = r"<path-of-image-directory>"
    
    map_name = '<map_name>'

    main(input_trajectory_file, images_directory, url, token, map_name)
